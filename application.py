...
def inserir(titulo):
    ...
    if request.method == "POST":
        if not getreview:
            ...
            alo1 = db.execute("SELECT book_id FROM titles WHERE title = :title", {"title": titulo })
            if db.execute("SELECT * FROM review WHERE usuario = :usuario AND review_book_id = :review_book_id", {"review_book_id": alo1.fetchone().book_id, 'usuario': session['logged_in']}).rowcount != 0:
                return render_template('error.html', mensagem11 = 'alos')
            else:
                alo = db.execute("SELECT book_id FROM titles WHERE title = :title", {"title": titulo })
                db.execute("INSERT INTO review VALUES(:rev, :review_book_id, :usuario, :date)", {"review_book_id": alo.fetchone().book_id, "rev": getreview, "usuario": session['logged_in'], "date": datetime.now()})
                db.commit()
                return redirect(url_for('books', titulo = titulo))
    else:
        return redirect(url_for('books', titulo = titulo))
...
def rate(titulo):
    getrate = request.form["rate"]
    if request.method == "POST":
        if not getrate:
            return redirect(url_for('books', titulo = titulo))
        else:
            alo1 = db.execute("SELECT book_id FROM titles WHERE title = :title", {"title": titulo })
            db.execute("INSERT INTO rate VALUES(:rate, :rate_id)", {"rate": getrate, "rate_id": alo1.fetchone().book_id})
            db.commit()
            return redirect(url_for('books', titulo = titulo))
    else:
        return redirect(url_for('books', titulo = titulo))
...
def api1():
    isbn = request.form.get("api")
    if not isbn:
        return render_template("api.html")
    else:
        return redirect(url_for('api', isbn = isbn))


@app.route("/api/<isbn>", methods=['GET'])
def api(isbn):
    alo = db.execute("SELECT * FROM isbn JOIN titles ON id = book_id JOIN author ON id = author_book_id WHERE isbn = :isbn", {'isbn': isbn})
    if alo.rowcount == 0:
        return jsonify({"error"}), 422
    else:
        book = alo.fetchone()
        review = db.execute("SELECT * FROM review WHERE review_book_id = :book_id", {'book_id': book.id})
        rate = db.execute("SELECT * FROM rate WHERE rate_id = :book_id", {'book_id': book.id})
        if rate.rowcount == 0:
            message2 = 0
        else:
            j, somador =  0, 0
            while j < rate.rowcount:
                ...
                j = j + 1
            message2 = somador/rate.rowcount
        return jsonify({"title": book.title, "author": book.author, "year": book.year, "isbn":book.isbn, "avarage_score":message2, "review_count": review.rowcount})
